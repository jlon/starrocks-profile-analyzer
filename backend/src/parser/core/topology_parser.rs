        ...
        if let Some(sink_name) = selected_sink {
            ...
            if !nodes.iter().any(|n| n.id == sink_id) {
                let sink_node = TopologyNode {
                    ...
                    children: vec![], // SINK 节点作为叶子节点，没有子节点
                };
                nodes.push(sink_node);

                // 建立父子关系：将 SINK 节点连接到拓扑结构的根节点
                if let Some(root_idx) = nodes.iter().position(|n| n.id == root_id) {
                    nodes[root_idx].children.push(sink_id);
                }
            }
        }
