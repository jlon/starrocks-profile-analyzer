...
impl SectionParser {
    ...
    fn extract_block(text: &str, section_marker: &str) -> ParseResult<String> {
        if let Some(start) = text.find(section_marker) {
            ...
            let base_indent = Self::get_indent(lines.first().copied().unwrap_or(""));
            
            let mut end_pos = rest.len();
            for (i, line) in lines.iter().enumerate().skip(1) {
                if !line.trim().is_empty() {
                    let curr_indent = Self::get_indent(line);
                    if curr_indent <= base_indent && line.trim().ends_with(':') {
                        end_pos = lines[..i].join("\n").len();
                        break;
                    }
                }
            }
    ...
    fn parse_total_time_ms(time_str: &str) -> Option<u64> {
