...
impl MetricsParser {
    /// 解析通用指标块
    /// 
    /// 从 Operator 文本中提取 CommonMetrics 部分并解析。
    /// 
    /// # Arguments
    ...
    pub fn parse_common_metrics(text: &str) -> OperatorMetrics {
        let common_block = Self::extract_common_metrics_block(text);
        Self::parse_metrics_from_text(&common_block)
    }
    ...
    pub fn parse_metric_line(line: &str) -> Option<(String, String)> {
        // 跳过 __MIN_OF_ 和 __MAX_OF_ 行（这些是聚合后的辅助指标）
        if line.contains("__MIN_OF_") || line.contains("__MAX_OF_") {
            return None;
        }
        ...
        match key {
            "OperatorTotalTime" => {
                ...
                if let Ok(duration) = ValueParser::parse_duration(value) {
                    metrics.operator_total_time = Some(duration.as_millis() as u64);
                }
            }
            // 优先使用 MAX 值覆盖（与 SR 官方解析一致）
            ...
            "PushTotalTime" => {
                if let Ok(duration) = ValueParser::parse_duration(value) {
                    metrics.push_total_time = Some(duration.as_millis() as u64);
                }
            }
            ...
            "PullTotalTime" => {
                if let Ok(duration) = ValueParser::parse_duration(value) {
                    metrics.pull_total_time = Some(duration.as_millis() as u64);
                }
            }
...
mod tests {
    ...
    fn test_skip_min_max() {
        ...
        assert!(MetricsParser::parse_metric_line("  - __MAX_OF_PullRowNum: 100").is_none());
    }
