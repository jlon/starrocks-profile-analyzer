    ...
    fn extract_operators(text: &str) -> Vec<Operator> {
        ...
        while i < lines.len() {
            ...
            if OperatorParser::is_operator_header(trimmed) {
                // 找到 operator header
                let operator_name = trimmed.trim_end_matches(':').to_string();
                ...
                // 提取 plan_node_id (从 operator_name 中解析，如 "CONNECTOR_SCAN (plan_node_id=0)")
                let plan_node_id = if operator_name.contains("plan_node_id=") {
                    operator_name
                        .split("plan_node_id=")
                        .nth(1)
                ...
                // 提取 CommonMetrics 和 UniqueMetrics
